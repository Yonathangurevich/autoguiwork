...
pub enum EngineErrorKind {
    ...
    #[error("this automation requires the input '{name}', but the request did not provide it")]
    ...
    #[error("no automation is exposed at endpoint '{endpoint_id}'")]
    EndpointNotFound { endpoint_id: String },

    #[error("the automation worker is not available")]
    WorkerUnavailable,

    #[error("could not start the worker server on '{addr}': {source}")]
    ServerBind {
        addr: String,
        #[source]
        source: std::io::Error,
